...
unsigned char w25qxx_read_state_register()
{
	unsigned char tmp;
	GPIO_ResetBits(P03);
...
{   
	while((w25qxx_read_state_register()&0x01)==0x01);  
}
...
unsigned int w25qxx_read_id(void)
{
	...
	spi_read_write_byte(0x00);
	spi_read_write_byte(0x00);
	tmp |= spi_read_write_byte(0xff)<<8;
	tmp |= spi_read_write_byte(0xff);
	GPIO_SetBits(P03);
	return tmp;
}
...
void w25qxx_read(unsigned char *pBuffer, unsigned int ReadAddr, unsigned int NumByteToRead)
{
	...
	spi_read_write_byte((unsigned char)(ReadAddr>>16)&0xff);
	spi_read_write_byte((unsigned char)(ReadAddr>>8)&0xff);
	spi_read_write_byte((unsigned char)ReadAddr&0xff);
...
unsigned char xdata w25qxx_buffer[4096];		 
...
void w25qxx_erase_sector(unsigned int addr)
{
	...
	spi_read_write_byte((unsigned char)(addr>>16)&0xff);
	spi_read_write_byte((unsigned char)(addr>>8)&0xff);
	spi_read_write_byte((unsigned char)addr&0xff);
	GPIO_SetBits(P03);
	w25qxx_wait_busy();
}
